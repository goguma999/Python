# 1. wrap : 문자열을 특정 문자로 감싸는 함수
# 2. analyze_sentiment : 감정 분석 함수
# 3. hankook(키워드,페이지수) : 한국일보 데이터 수집
# 4. dongah(키워드,페이지수) : 동아일보 데이터 수집
# 5. ja(키워드,페이지수) : 중앙일보 데이터 수집
# 6. han(키워드,페이지수) : 한겨례 데이터 수집
# 7. km(키워드,페이지수) : 국민일보 데이터 수집
# 8. chosun(키워드, 페이지수) : 조선일보 데이터 수집
# 9. naver_blog2(키워드, 페이지수) : 네이버 블러그 데이터 수집
# 10. naver_shopping(키워드, 페이지수) : 네이버 쇼핑 데이터 수집
# 11. get_coupang_data(키워드, 페이지수) : 쿠팡 데이터 수집
# 12. download_image_bing(kw) : 빙 이미지 수집, c://data_image_bing에 저장됨
# 13. download_image_naver(kw) : 네이버 이미지 수집, c://data_image_naver에 저장됨
# 14. download_image_google(kw) : 구글 이미지 수집, c://data_image_google에 저장됨
# 15. download_image_daum(kw) : 구글 이미지 수집, c://data_image_daum에 저장됨
# 16. scroll_youtube(kw) : 유튜브 댓글 수집, c://data에 저장됨
# 17. relation_word_ja(kw) : 중앙일보 2024 언급량 시각화


# 1. 문자열을 특정 문자로 감싸는 함수
def wrap(text, char="'"):
...
def josun(kw, num):
    ...
    for i in result:
        driver.get(i)  #상세 기사 url을 하나씩 열기
        time.sleep(5) 
...
def naver_blog(kw, n):
    ...
    for i in range(1,n+1):
        ...
        #크롬 로봇이 웹페이지를 직접 열 수 있게 함
        driver.get(list_url)
        time.sleep(2)
        
        #크롬 로봇이 직접 연 웹페이지의 html 코드를 가져옴
...
def  youtube_page_html_source(urls):
    ...
    html_sources = [] # html 코드를 저장하기 위한 리스트 생성

    # 크롬 로봇을 지정합니다.
    service = Service(ChromeDriverManager().install())
    driver = webdriver.Chrome(service=service)

    for  i  in  range(0, 20): # 전체 url 말고 2개의 url 만 가져옴 
        
        # 크롬 로봇이 url 을 직접 열게 함
        driver.get(url[i])
    
        # 키워드로 받아온 영상 리스트 웹페이지의 처음부터 맨 마지막까지의 높이를 추출
        last_page_height = driver.execute_script("return document.documentElement.scrollHeight")
    
        while True:  #   아래의 실행문이 무한히 반복될 수 있도록 함
        ...
        time.sleep(5)
        html_source = driver.page_source  # 페이지의 html 소소를 html_source 에 담기
        time.sleep(5)
...
def relation_word_ja(keyword):    
    ...
    # 텍스트 파일 열기 (with절을 써서 열면 파이썬이 알아서 파일을 닫아줌)
